...
func (r *Repo) Create(ctx context.Context, pa *entity.PaymentAttempt) error {
	query := `
		INSERT INTO payment_attempts (payment_id, bank_id, created_at, updated_at)
		VALUES ($1, $2, $3, $4)
		RETURNING id
	`
	row := r.database.QueryRowContext(ctx, query, pa.PaymentId, pa.BankId, pa.CreatedAt, pa.UpdatedAt)
	if err := row.Scan(&pa.Id); err != nil {
		return err
	}
	return nil
}
